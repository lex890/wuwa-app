...
export default function TierList({ data }) {
  ...
  const tierRows = useMemo(() => {
    const sorted = [...filteredCharacters].sort(
    ...
    const rows = Object.fromEntries(
      TIER_ROWS.map((tier) => [
        tier,
        Object.fromEntries(ROLE_COLUMNS.map((col) => [col, []]))
      ])
    );
    ...
    sorted.forEach((character, index) => {
      ...
      const tier = TIER_ROWS[rowIndex];
      const col = getRoleColumn(character);

      rows[tier][col].push(character);
    });

    return rows;
  }, [filteredCharacters, explicitTierMap]);
